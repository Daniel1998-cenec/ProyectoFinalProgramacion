...
public class PantallaAnime extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		try {
			BufferedImage fondo=ImageIO.read(new File(".\\img\\fondo5.jpg"));
	...
	public PantallaAnime(Ventana v){
		...
		buttonDeporte.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String aux="<html>";
				try {
					todo=RuletaRusa.devolverGeneroAnime(GeneroAnime.DEPORTE);
					for(byte i=0;i<todo.size();i++) {
						aux+="<p>"+todo.get(i)+"</p>";
					}
					aux+="</html>";
					labelText.setText(aux);
				} catch (GeneroAnimeException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
			}
		...
		gbc_buttonAventura.insets = new Insets(0, 0, 5, 5);
		gbc_buttonAventura.gridx = 2;
		...
		buttonShounen.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String aux="<html>";
				try {
					todo=RuletaRusa.devolverGeneroAnime(GeneroAnime.SHOUNEN);
					for(byte i=0;i<todo.size();i++) {
						aux+="<p>"+todo.get(i)+"</p>";
					}
					aux+="</html>";
		...
		gbc_buttonShounen.insets = new Insets(0, 0, 5, 0);
		gbc_buttonShounen.gridx = 4;
